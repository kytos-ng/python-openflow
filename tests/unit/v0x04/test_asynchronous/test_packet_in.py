...
class TestPacketInRaw(StructTest):
    """Test PacketIn using a dump file."""

    def setup_method(self):
        """Configure raw file and its object in parent class (TestDump)."""
        self.set_raw_dump_file('v0x04', 'ofpt_packet_in')
        self.set_raw_dump_object(PacketIn, xid=0, buffer_id=OFP_NO_BUFFER,
                                    total_len=90,
                                    reason=PacketInReason.OFPR_ACTION,
                                    table_id=0, cookie=0x0000000000000000,
                                    match=_new_match(), data=_get_data())
        self.set_minimum_size(34)
